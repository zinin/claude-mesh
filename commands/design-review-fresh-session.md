...
<!-- SYNC: the DO NOT / ENVIRONMENT / PREFLIGHT / THEN STOP blocks of the generated prompt are
     ...
     neither. The `default`-safety rule under WHEN THE USER SAYS GO is mirrored there too, with
     `design_review` swapped for `code_review` and nothing else.
...
Split both on `, ` and compare WHOLE entries — never substrings: `claude` is a substring of
`claude:opus` and `zai/glm` of `zai/glm-air`, so a substring test reports a match where the
orchestrator would find none.
