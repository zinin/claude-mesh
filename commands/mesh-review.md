...
**Before dispatch — stamp the delegation window (Step 6.0 guard needs it).** Via a Bash tool call, record `DISPATCH_EPOCH=$(date +%s)` and keep the number. **HOST=grok: in the same call, also stamp the tree** — `TREE_BEFORE=$( { git status --porcelain --untracked-files=all; git diff HEAD; } | sha256sum | cut -d' ' -f1 )` — and keep it. Native children are `general-purpose` (they need a shell for `git diff`), so nothing but the prompt's «Do not edit files» keeps them from writing; this hash is the mechanical check behind that sentence. Also remember the list of *wrapper* reviewers being dispatched as `engine:model` pairs: `codex`→`codex:-`, `gemini`→`gemini:-`, each entry of `SELECTED_GROK_MODELS`→`grok:<model>` (`grok:grok-4.6` — a colon, and the bare catalog id), each selected model id→`ext-claude:<id>`. `grok` with an empty `SELECTED_GROK_MODELS` contributes no pair at all: nothing was dispatched, so there is nothing to verify.
...
- **Tree check (native):** when the last native child has finished, recompute the same hash as `TREE_AFTER`. If it differs from `TREE_BEFORE`, run `git status --porcelain --untracked-files=all` and carry that list into Step 6.0. Your own edits, and the user's edits made while the review ran, are the innocent explanation — say so; anything else was written by a native child and must be looked at before Step 6.2 commits auto-fixes on top of it. Never revert on your own.
...
**HOST=grok — tree row.** When `TREE_AFTER` differed from `TREE_BEFORE` (Step 5a), add one row `tree | CHANGED | ⚠ файлы изменились во время ревью: <list from git status> — если это не ваши правки, их внёс native-ребёнок; проверьте до Step 6.2`. Equal hashes: no row. This is a report, not a verdict on any reviewer, and it prescribes no revert.
