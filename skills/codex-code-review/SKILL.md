...
if [ -n "$SKILL_BASE" ]; then
  ...
  SKILL_BASE="$PLUGIN_ROOT/skills/codex-code-review"
fi
...
Shared scripts live at `$SKILL_BASE/../shared/<x>` (e.g. `code-review-prompt.md`); the loader is `$SKILL_BASE/../shared/config-loader.sh`. Codex manages its own auth — this skill does NOT source provider tokens; it delegates execution to the `codex-exec` skill.
...
`find "$HOME"/.claude/plugins -path '*claude-mesh*/skills/codex-exec/SKILL.md' 2>/dev/null | sort -V | tail -1` — and, only if that prints nothing, `find "$HOME"/.grok/plugins -path '*claude-mesh*/skills/codex-exec/SKILL.md' 2>/dev/null | sort -V | tail -1`.
