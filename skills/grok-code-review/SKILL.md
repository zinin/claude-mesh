...
`find "$HOME"/.claude/plugins -path '*claude-mesh*/skills/grok-exec/SKILL.md' 2>/dev/null | sort -V | tail -1` — and, only if that prints nothing, `find "$HOME"/.grok/plugins -path '*claude-mesh*/skills/grok-exec/SKILL.md' 2>/dev/null | sort -V | tail -1`.
