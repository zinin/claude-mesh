...
- **`check-context-size.sh` reads Grok `signals.json` (`contextTokensUsed`) and keys the session off `sessionId` / `$GROK_SESSION_ID`.** Data dir falls back to `$GROK_PLUGIN_DATA`. Subagent fires (`subagentType` or `agent_id`) stay silent. Registered on `PreToolUse` as well as `PostToolUse` (Grok ignores PostToolUse stdout); both handlers set `timeout: 10`.
